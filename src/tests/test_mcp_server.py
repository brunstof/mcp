...
class TestMariaDBMCPTools(unittest.IsolatedAsyncioTestCase):
        ...
        async with anyio.create_task_group() as tg:
            ...
            async with self.client:
                result = await self.client.call_tool(
                    "get_table_schema", {"database_name": "information_schema", "table_name": "ALL_PLUGINS"}
                )
                result = result.text
                result = json.loads(result)
            ...
            async with self.client:
                try:
                    await self.client.call_tool(
                        "get_table_schema", {"database_name": "information_schema", "table_name": "INVALID_TABLE"}
                    )
    ...
    async def test_execute_sql(self):
        async with anyio.create_task_group() as tg:
            ...
            async with self.client:
                result = await self.client.call_tool(
                    "execute_sql", {"database_name": "information_schema", "sql_query": "SELECT 1"}
                )
                result = result.text
                result = json.loads(result)
                self.assertIsInstance(result, list)
            ...
            async with self.client:
                try:
                    await self.client.call_tool(
                        ...
                        {
                            "database_name": "information_schema",
                            "sql_query": "SELECT * FROM information_schema.tables WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s",
                            "parameters": ["information_schema"],
                        },
                    )
                    self.fail("Expected an exception, but none was raised")
    ...
    async def test_create_vector_store(self):
        async with anyio.create_task_group() as tg:
            ...
            async with self.client:
                ...
                result = await self.client.call_tool(
                    "create_vector_store", {"database_name": "test_database", "vector_store_name": "test_vector_store"}
                )
                result = result[0].text
                result = json.loads(result)
                self.assertIsInstance(result, dict)
    ...
    async def test_delete_vector_store(self):
        async with anyio.create_task_group() as tg:
            ...
            async with self.client:
                ...
                result = await self.client.call_tool(
                    "delete_vector_store", {"database_name": "test_database", "vector_store_name": "test_vector_store"}
                )
                result = result[0].text
                result = json.loads(result)
                self.assertIsInstance(result, dict)
